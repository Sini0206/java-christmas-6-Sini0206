...
## 📝로직 요약 
- 가능한 주문이면 주문 정보를 저장한다.
- 이벤트 대상자 자격을 충족한다면, 날짜와 총 주문 금액에 따라 분류한 이벤트 중 가능한 이벤트를 적용한다.
- 혜택 금액에 따라 배지를 부여한다.
...
- [x] 입력 받은 내용을 토대로 주문 생성

- [x] 주문 가능 여부 확인
  - [x] 음료만 주문 시 주문 불가
  - [x] 주문수량 확인 (메뉴 최대 20개까지 가능)


- [x] 이벤트 대상자 자격 여부 판단 : 10,000원 이상일 시 이벤트 적용
- [x] 날짜가 이벤트 기간에 포함되는지 확인
  - 12월 이벤트 - (이벤트 기간: 12월 1일 ~ 12월 31일)
  - 크리스마스 디데이 할인 - (이벤트 기간: 12월 1일 ~ 12월 25일)
- [x] 적용 가능한 이벤트 판단(중복 적용 가능)


- [x] 이벤트 분류
  - [x] 증정 이벤트 : 할인 전 총주문 금액이 12만원 이상이면 샴페인 1개 증정
  - [x] 할인 이벤트 :
    - [x] 12월 이벤트 :
      - [x] 평일 할인 (날짜가 토요일도 일요일도 공휴일도 아님)
        - 디저트 메뉴 1개 당 2,023원 할인
      - [x] 주말 할인 (날짜가 토요일이거나 일요일)
        - 메인 메뉴 1개 당 2,023원 할인
      - [x] 특별 할인
        - 날짜가 공휴일(일요일이거나 크리스마스)이면 총 주문 금액에서 1000원 할인
    - [x] 크리스마스 디데이 할인
      - 할인 금액: 1000원에서 시작하여 날마다 할인 금액 100원씩 증가
...
- [x] 12월 이벤트 플래너
  - [x] 입력 사항 : 
    - [X] 날짜
    - [X] 메뉴이름, 주문수량 Ex)메뉴이름-주문수량(쉼표로 구분)
  - [x] 출력 사항 : 
    - [x] 주문 메뉴 Ex)메뉴이름 주문수량(개)
    - [x] 총 주문 금액 
    - [x] 증정 메뉴 
    - [x] 혜택 내역 
      - 크리스마스 디데이 할인, 평일 할인, 주말 할인, 특별 할인, 증정 이벤트 순
    - [x] 총 혜택 금액 
    - [x] 할인 후 예상결제 금액 
    - [x] 이벤트 배지
...
- [ ] 예외 처리 
  ...
  - [x] 방문 날짜가 1에서 31 사이의 숫자가 아닌 경우
    - "[ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요."
  - 고객이 메뉴판에 없는 메뉴를 입력하는 경우
    - "[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."
  - [X] 메뉴의 개수는 1 이상의 숫자만 입력 되도록
    - "[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."
  - 메뉴의 형식이 예시와 다를 경우 
    - "[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."
